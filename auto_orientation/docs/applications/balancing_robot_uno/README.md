...
**Status**: Phase 4U landing doc — stub. Source scaffold owned by sibling agent (`src/applications/balancing_robot_uno/`). Python brute-force tuner owned by sibling agent (`tools/sim/`).
...
> Planned layout — sibling agent owns the scaffold.

```
src/applications/balancing_robot_uno/
├── balance_uno_app.h            # Top-level API
├── balance_uno_app.cpp          # PID loop, ~150 LOC ceiling
└── generated/
    └── balance_constants_uno.h  # constexpr Kp/Ki/Kd/PWM, generated by Python tuner
```
