...
| D7 | Online drift tracking algorithm | ~~Slow LPF of integral term~~ → **AS-BUILT: slow LPF of *mean pitch*** on AVR Mega; 3-state Kalman extension reserved on Teensy/ESP32 | [online_adaptive_balance_tracking.md](online_adaptive_balance_tracking.md) |
...
| D9 | Auto-PID default strategy | ~~Amplitude-limited relay feedback (Åström-Hägglund 1984)~~ → **AS-BUILT: BOOTSTRAP ±PWM-pulse K_motor measurement → critically-damped pole-placement → continuous scalar RLS.** Relay tuner is compiled out / unwired. | [auto_pid_tuning_research.md](auto_pid_tuning_research.md) |
...
## AS-BUILT vs AS-DESIGNED (2026-05-22)

The control/auto-tune stack has evolved past the design rows above. Each item below was **verified line-by-line against the current `src/`** (see [`autocal_autotune_verification_2026-05-22.md`](autocal_autotune_verification_2026-05-22.md)). These are deliberate, defensible engineering choices — not bugs — but the doc rows are stale, so they are reconciled here. The math of every as-built path was checked and is correct.

| # | What the design says | What the firmware actually runs | Source of truth |
|---|----------------------|---------------------------------|-----------------|
| AB1 | **D9 / bootstrap Stage 4:** AMIGO mapping (`Kp=0.45·Ku`, `Ki=Kp/(0.85·Tu)`, `Kd=Kp·0.125·Tu`) | **Critically-damped pole-placement**: `Kp=ωₙ²/K_motor`, `Kd=2ζωₙ/K_motor`, `ωₙ=4/ts`, `ζ=0.7`. AMIGO survives only in the compiled-out relay tuner. | `plant_identifier.cpp:262-282`, `recompute_targets_()` |
| AB2 | **bootstrap §5:** explicit `BootstrapStage{SEED, MOUNT_CONVERGED, PLANT_IDENTIFIED, GAINS_REFINED, ADAPTIVE}` enum + `try_advance_bootstrap_()` staged state machine + per-stage completion rules + LED encoding + EEPROM write-back of refined K/offset | **No enum, no staged machine.** Collapsed to a single `BOOTSTRAP` app-state doing **direct ±PWM pulse identification** (replacing "Stage 3 = RLS from natural disturbance"), then `RUN` runs continuous RLS + 5%/s gain ramp + mount estimator. Cleaner for an unstable plant (direct pulse K beats waiting for natural excitation). | `balance_app.cpp` — `step_bootstrap_*`, no `BootstrapStage` symbol exists |
| AB3 | **D7 / bootstrap Stage 2:** mount estimator LPFs the inner-loop **I-term** (`mount_offset += alpha·(i_term·gain_to_angle)`) | **LPFs mean pitch directly** (`target_deg = pitch_deg`). I-term path abandoned (±0.4° range, averaged to zero under oscillation). `i_term`/`gain_to_angle_` are dead `(void)`-cast no-ops kept for API stability. | `online_mounting_estimator.cpp:112-191` |
| AB4 | **phase_4m14 §2.2:** position-loop `g_lean = g_eff·(π/180)·r` (yields K_POS≈2000, ~300× wrong) | **`g_lean = 9.81`** (physically-correct small-angle inverted-pendulum `a≈gθ`), giving `G_outer = g·(π/180)` and K_POS≈5.8 (~1.03× the 4M.13 hand-pick of 6.0). Self-documented deliberate deviation. The §2.2 spec formula is **wrong**; the as-built is the accepted approach. Pole-placement formulas in §3 are correct and used as-is. | `balance_app.cpp:1549-1645`, `derive_position_gains_()` |
| AB5 | (not in original design) | **New `noise_floor_estimator.h`** — Welford running σ of gyro-rate / accel-deviation over a 200-sample (1 s) quiet window. **Observation-only; nothing downstream consumes it yet** (by design). It is the missing "measure, don't hardcode" feedstock that *unblocks the measurement side* of the 5 noise-cited scope violations (`STUCK_GYRO_DPS`, `ext_motion gyro>30`, HELD `a_dev_lpf_>6`, HELD→RUN gate, HELD dwell σ) — still bench-gated to actually wire up. | `src/applications/balancing_robot/noise_floor_estimator.h` |
| AB6 | mount-estimator `lpf_tc` default 300 s (code) / 60 s (bootstrap doc) | **Overridden to 8 s** at `main.cpp:588` (and `max_drift_rate` to 2.0°/s at `main.cpp:589` vs 0.5°/s D8 default). The triage doc #13 flags 8 s as **questionable** for a slow-drift tracker. Bench-gated to change. | `main.cpp:588-589` |

The auto-tune chain that actually runs end-to-end is therefore: **BOOTSTRAP ±PWM pulses → measured `K_motor` → `seed_k_motor()` → pole-placement Kp/Kd → continuous scalar RLS (λ=0.998) with σ-modification → 5%/s gain ramp → `set_tunings()`**, with the **position outer loop** gains derived once at BOOTSTRAP finalise (also pole-placement, `g_lean=9.81`), and the **mean-pitch mount estimator** + **noise-floor estimator** running alongside in `RUN`.
