...
**Last updated**: 2026-05-26 (Uno SETUP MODE + on-Uno BNO055 cal + P→D→I — see [archive/session_records/2026-05-26_uno_setup_mode.md](archive/session_records/2026-05-26_uno_setup_mode.md))

### 2026-05-26 status snapshot

Uno-only session — Mega side unchanged. **Uno setup mode + on-Uno BNO055 cal + P→D→I + photo-backup printer — shipped 2026-05-26 (uncommitted).** Two-env split: `arduino_uno_tuning` (SETUP MODE — calibrate + guided tune) vs `arduino_uno_minimal` (OPERATIONAL MODE — lean flight, reads EEPROM). On-Uno calibration via new `'c'` retires the Mega-path cal dependency; stage order reordered P→I→D → P→D→I; every EEPROM-written value also printed as a photo-backup block for hardcode-paste recovery. Builds GREEN (`arduino_uno_tuning` 73 % flash, `arduino_uno_minimal` 63 % flash, `mega_balance` unchanged); native 19/19; all 8 audit categories GREEN. Bench-validation of the new operator UX is now part of the bench gate. Session record: [archive/session_records/2026-05-26_uno_setup_mode.md](archive/session_records/2026-05-26_uno_setup_mode.md).
...
*Last updated: 2026-05-26 (Uno SETUP/OPERATIONAL mode split shipped — `arduino_uno_tuning` 73 % flash, `arduino_uno_minimal` 63 % flash; on-Uno BNO055 cal via new `'c'` + `calibration_session.{cpp,h}` retires the Mega-path cal dependency; guided PID stage order reordered **P→I→D → P→D→I**; photo-backup printer `tune_storage::print_photo_backup()` emits copy-paste-ready block on `'w'`/`'s'`, `balance_constants.h` gained PHOTO-BACKUP HARDCODE SITE + `BNO055_CAL_BLOB[22]` decl; Mega side unchanged; native 19/19; all 8 audit categories GREEN; uncommitted in working tree. Bench-validation of the new Uno operator UX is now part of the bench gate. Session record: [archive/session_records/2026-05-26_uno_setup_mode.md](archive/session_records/2026-05-26_uno_setup_mode.md). Prior: 2026-05-22 safety/correctness/docs session — NaN-safety failsafes across the motor path, `noise_floor_estimator.h` observation layer, quaternion gimbal-lock production fix, ASCII→Mermaid + `docs/architecture/` LEVEL_0/1/2 + as-built reconciliation, Uno link regression fixed via one-line `platformio.ini` src_filter, native 22/22.). When a phase enters or exits, update both this file and `todo.md`. Per-session work logs go to `docs/archive/session_records/`.*
