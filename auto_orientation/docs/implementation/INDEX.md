...
| [auto_pid_tuner.md](auto_pid_tuner.md) | `src/control/auto_pid_tuner.{h,cpp}` + `tuning_strategy.h` + `tuners/relay_feedback.{h,cpp}` | Auto-PID coordinator + Åström-Hägglund relay-feedback strategy; pluggable `ITuningStrategy`. |
...
| [online_mounting_estimator.md](online_mounting_estimator.md) | `src/navigation/online_mounting_estimator.{h,cpp}` | Slow-LPF online drift tracker (AVR path); ±5° hard bound, 0.5°/s rate limit, 4 freeze gates. |
