...
struct AutoOrientRecord {
    ...
    uint8_t crc8;                   ///< XOR checksum of bytes 0..22
};

static const uint8_t  AUTO_ORIENT_RECORD_MAGIC   = 0xAB;
static const uint8_t  AUTO_ORIENT_RECORD_VERSION = 0x01;
static const size_t   AUTO_ORIENT_RECORD_SIZE    = 24;

/**
 * CRC8 helper — simple XOR, identical algorithm to
 * `calculateCRC8` in `src/config/calibration_storage.cpp` but kept local
 * to avoid pulling in EEPROM headers (and to side-step a potential
 * circular-include with a parallel storage rewrite).
 */
