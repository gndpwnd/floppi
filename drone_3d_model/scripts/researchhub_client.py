...
def cmd_workspace(args, config):
    ...
    if sub == "list":
        ...
        if args.origin_url:
            payload["origin_url"] = args.origin_url
        action = "Previewing" if args.dry_run else "Merging"
        print(f"{action} workspace '{ws}' with origin...")
        data = api_post(url, f"/api/workspaces/{ws}/merge?source=origin", payload, timeout=120)
        status = data.get("status", "unknown")
        if status == "error":
            die(f"Merge failed: {data.get('error', 'unknown error')}")
        if args.dry_run:
            print("DRY RUN — no changes made:")
        else:
            print(f"Merge {status}:")
        stats = data.get("stats", {})
        rows = [[k.replace("_", " ").title(), str(v)] for k, v in stats.items() if v]
        if rows:
            out_table(rows, ["Metric", "Count"])
        else:
        ...
        if conflicts:
            ...
            for c in conflicts[:20]:
                print(f"  [{c.get('table')}] {c.get('record_id', '?')[:20]}: "
                      f"{c.get('resolution')} -> {c.get('winner')}")
            if len(conflicts) > 20:
        ...
        if not ws:
            die("Workspace name required (--name or RESEARCHHUB_WORKSPACE)")
        # --no-keep-papers inverts --keep-papers (default True).
        # Previously the flag was parsed but never read, so passing
        # --no-keep-papers was a silent no-op.
        keep_papers_effective = getattr(args, "keep_papers", True)
        if getattr(args, "no_keep_papers", False):
            keep_papers_effective = False
        # Build query params
        params = {
            "confirm": "true",
            "keep_papers": str(keep_papers_effective).lower(),
...
def cmd_init(args, config):
    ...
    if getattr(args, "client_script_path", None):
        conf["client_script_path"] = args.client_script_path
    else:
        # Auto-detect: record relative path to THIS script from project root
        try:
            this_script = Path(__file__).resolve()
            project_root = Path.cwd().resolve()
            if this_script.is_relative_to(project_root):
                conf["client_script_path"] = str(this_script.relative_to(project_root))
            else:
                conf["client_script_path"] = "researchhub_client.py"
...
def _gitignore_fix_root(repo):
    """Fix broken patterns in repo-root .gitignore that ignore managed folders."""
    ...
    # Build set of managed folder paths
    managed_paths = {}
    all_folders = list(_MANAGED_FOLDERS_CLIENT.keys()) + ["PDFs"]
    for rel_folder in all_folders:
        managed_paths[rel_folder] = rel_folder
        managed_paths[rel_folder + "/"] = rel_folder
        managed_paths[rel_folder + "/**"] = rel_folder
    ...
    for line in lines:
        ...
        if not stripped or stripped.startswith("#") or stripped.startswith("!"):
            ...
            continue
        if stripped in managed_paths:
            rel_folder = managed_paths[stripped]
            print(f"  .gitignore root — removing broken rule '{stripped}' (was ignoring {rel_folder})")
            negations_to_add[rel_folder] = True
            if rel_folder not in fixed_folders:
                fixed_folders.append(rel_folder)
            continue
        new_lines.append(line)
    ...
    for rel_folder in negations_to_add:
        neg_dir = f"!{rel_folder}/"
        neg_glob = f"!{rel_folder}/**"
        existing_stripped = {l.strip() for l in new_lines}
        if neg_dir not in existing_stripped:
            new_lines.append(neg_dir)
        if neg_glob not in existing_stripped:
            new_lines.append(neg_glob)

    new_content = "\n".join(new_lines)
    if not new_content.endswith("\n"):
        new_content += "\n"
    root_gi.write_text(new_content)
    print(f"  .gitignore root — fixed {len(fixed_folders)} broken pattern(s): {', '.join(fixed_folders)}")
    return len(fixed_folders)
...
def _gitignore_fix(repo_path):
    ...
    # First: fix broken patterns in repo-root .gitignore
    root_fixed = _gitignore_fix_root(repo)
    ...
    root_msg = f", {root_fixed} root pattern(s) fixed" if root_fixed else ""
    print(f"\nDone: {created} created, {updated} updated, {skipped} folders not present{root_msg}")
...
def cmd_git_cleanup(args, config):
    ...
    if sub == "preview":
        ...
        # Wired to POST /api/git-cleanup/execute/{repo_name} (server-side).
        # Only the db mode has an in-process executor (see
        # routers/git_cleanup.py:modular_execute); pdf cleanup is refused
        # server-side and must be run via scripts/cleanup_pdf_history.sh
        # locally so destructive prompts are visible.
        repo_name = getattr(args, "repo", None) or getattr(args, "name", None)
        mode = getattr(args, "mode", None) or "db"
        dry_run = bool(getattr(args, "dry_run", False))
        compress_flag = bool(getattr(args, "compress", False))
        force_push_flag = bool(getattr(args, "force_push", False))
        path_flag = getattr(args, "path", None)

        if dry_run:
            # Delegate to preview endpoint for a non-destructive audit.
            if not repo_name:
                die("--repo <name> is required with --dry-run "
                    "(server-side preview uses repositories.json names).")
            try:
                data = api_get(
                    url,
                    f"/api/git-cleanup/preview/{repo_name}",
                    params={"mode": mode},
                    timeout=60,
                )
                out_json(data)
            except APIError as e:
                die(f"Preview failed: HTTP {e.status} {e.message}")
            return

        # Warn about pdf-only modes that the server refuses.
        if mode == "pdf":
            print("UNSUPPORTED: server-side executor only runs db-mode cleanup.")
            print("Run PDF history cleanup locally with:")
            if path_flag:
                print(f"  scripts/cleanup_pdf_history.sh "
                      f"{os.path.abspath(path_flag)}")
            else:
                print("  scripts/cleanup_pdf_history.sh /path/to/repo")
            if compress_flag:
                print("  (use --compress on that script for pikepdf compression)")
            if force_push_flag:
                print("  (use --force-push on that script to push after rewrite)")
            return

        if not repo_name:
            die("--repo <name> is required (must match an entry in "
                "repositories.json). Available: try `git-cleanup preview` first.")

        # Confirm destructive operation unless --yes.
        if not getattr(args, "yes", False):
            print(f"About to run DESTRUCTIVE cleanup (mode={mode}) on "
                  f"repo '{repo_name}'. This rewrites history.")
            try:
        ...
        try:
            data = api_post(
                url,
                f"/api/git-cleanup/execute/{repo_name}",
                payload,
                timeout=600,
            )
            out_json(data)
            if compress_flag or force_push_flag:
                print("")
                print("NOTE: --compress / --force-push are client-side-only "
                      "flags and have no effect on the server-side db-mode "
                      "executor. For PDF compression + force-push run "
                      "scripts/cleanup_pdf_history.sh locally.")
        except APIError as e:
            die(f"Execute failed: HTTP {e.status} {e.message}")
...
def cmd_update_self(args, config):
    """Update this client script from the ResearchHub server."""
    url = config["url"]

    # Check what we have locally
    this_script = Path(__file__).resolve()
    try:
        local_size = this_script.stat().st_size
    except OSError:
        local_size = 0
    ...
    try:
        master = api_get(url, "/api/client-scripts/master", timeout=10)
    except APIError as e:
        die(f"Cannot reach server: {e.message}")

    if not master.get("exists"):
        die("Master client script not found on server.")

    master_hash = master.get("hash", "")
    master_size = master.get("size", 0)

    # Compare local hash
    import hashlib
    try:
        local_hash = hashlib.sha256(this_script.read_bytes()).hexdigest()
    except (OSError, IOError):
        local_hash = ""

    if local_hash == master_hash:
        print(f"Client script is up to date ({local_size} bytes).")
        return

    print(f"Update available:")
    print(f"  Local:  {local_size} bytes (hash: {local_hash[:12]}...)")
    print(f"  Server: {master_size} bytes (hash: {master_hash[:12]}...)")

    if check_only:
        print("\nRun without --check to apply update.")
        return

    # Fetch the updated script from server
    try:
        result = api_post(url, "/api/client-scripts/update", timeout=30)
    except APIError as e:
        # If the bulk update endpoint does not help, try fetching the
        # master content directly and writing it ourselves.
        die(f"Update failed: {e.message}")

    # The server updated all tracked copies.  But THIS copy may not be
    # tracked if we are running from an ad-hoc location.  Let the server
    # do tracked copies, and we also replace ourselves.
    try:
        # Re-fetch master info (the server may expose the content via
        # the master endpoint, but for simplicity we ask the server to
        # update tracked copies and then we re-download the diff ourselves.)
        # For a zero-dependency client we just re-read the master endpoint.
        # In practice the safest approach is to ask the server to serve
        # the raw script file.
        script_url = f"{url}/api/client-scripts/download"
        try:
            raw = _request("GET", script_url, timeout=30)
            if isinstance(raw, bytes):
                this_script.write_bytes(raw)
                print(f"Updated {this_script} ({len(raw)} bytes).")
                return
        except APIError:
            pass  # download endpoint may not exist yet

        # Fallback: tell the user to copy manually
        print("Server updated tracked copies.  To update this copy manually:")
        print(f"  Copy the latest from your ResearchHub scripts/ directory to: {this_script}")

    except (OSError, IOError) as exc:
        die(f"Failed to write update: {exc}")
...
""",
    ...
    p_init.add_argument("--client-script-path", default=None,
                        help="Path to this client script relative to project root (default: researchhub_client.py)")

    # update-self
    p_update = sub.add_parser("update-self", help="Update this client script from the server")
    ...
    # workspace merge (deployment mode switch)
    p_ws_merge = ws_sub.add_parser("merge", help="Merge local data with origin (standalone->proxy switch)")
    p_ws_merge.add_argument("--name", help="Workspace name")
    p_ws_merge.add_argument("--origin-url", help="Override origin URL (defaults to ORIGIN_URL env)")
    p_ws_merge.add_argument("--dry-run", action="store_true", help="Preview only -- no changes made")
    ...
    p_gc_execute = gc_sub.add_parser(
        "execute",
        help="Execute cleanup (db-mode server-side; pdf-mode defers to local script)",
    )
    p_gc_execute.add_argument("--repo", help="Repository name (from repositories.json)")
    p_gc_execute.add_argument("--mode", choices=["pdf", "db", "both", "all"], default="db",
                              help="Cleanup mode (default: db). "
                                   "Only db is executed server-side; pdf prints guidance.")
    p_gc_execute.add_argument("--dry-run", action="store_true",
                              help="Preview only via /api/git-cleanup/preview/{repo} - no changes")
    p_gc_execute.add_argument("--yes", action="store_true",
                              help="Skip the destructive-confirmation prompt")
    p_gc_execute.add_argument("--path", help="Specific repo path (pdf-mode local hint only)")
    p_gc_execute.add_argument("--compress", action="store_true",
                              help="(client-side hint only; no effect on server db-mode executor)")
    p_gc_execute.add_argument("--force-push", action="store_true",
                              help="(client-side hint only; no effect on server db-mode executor)")
